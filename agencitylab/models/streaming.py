...
It is designed for near-real-time workflows. For true incremental-operator
optimizations, the core functions can later be migrated to streaming-specific
kernels.
"""
...
class AgencityStream:
    ...
    ----------
    window_size:
        Maximum number of recent samples kept in memory. If None, the full
        history is kept.
    preset:
        Compute preset name.
    config:
    ...
    def update(
        ...
        **kwargs,
        ...
        Returns the current AgencityResult.
        """
        if xi_chunk is None:
