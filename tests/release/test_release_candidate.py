...
def test_quickstart_compute_analyze_export_roundtrip(tmp_path):
    ...
    assert result.metadata.agencitylab_version == __version__ == "1.1.2"
    ...
    assert restored.metadata.agencitylab_version == "1.1.2"
...
def test_batch_preserves_order_and_per_item_physics():
    ...
    assert all(result.metadata.agencitylab_version == "1.1.2" for result in results)
