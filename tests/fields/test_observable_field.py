...
REST_ZERO_FIELDS = CANONICAL_FIELDS[1:]
...
def _trajectory(values: np.ndarray, time_axis: int, spatial_index: tuple[int, ...]):
    return np.moveaxis(values, time_axis, 0)[(slice(None), *spatial_index)]


def _assert_scalar_equivalence(result, spatial_index, scalar) -> None:
...
def test_single_spatial_point_is_exact_scalar_pipeline():
    t = _time()
    u = _signal(t)[:, None]
    field = compute_agencity_field(u, t, A_ref=1.3, tau=4.0, w=3.0, P_c=2.5)
...
def test_spatial_physical_parameter_matches_independent_scalar_calls(parameter, values):
    t = _time()
    u = np.column_stack([_signal(t, 0.2 * index) for index in range(3)])
    options = dict(A_ref=1.0, tau=4.0, w=3.0, P_c=1.0)
    options[parameter] = values
    field = compute_agencity_field(u, t, **options)
    for index, local_value in enumerate(values):
        local = dict(A_ref=1.0, tau=4.0, w=3.0, P_c=1.0)
        local[parameter] = float(local_value)
        scalar = compute_agencity(u=u[:, index], xi=t, **local)
        _assert_scalar_equivalence(field, (index,), scalar)


def test_spatial_w_is_independent_from_tau_and_fallback_is_explicit():
    t = _time()
    u = np.column_stack([_signal(t), _signal(t, 0.3)])
    tau = np.array([2.0, 5.0])
    explicit = compute_agencity_field(
        u, t, A_ref=1.0, tau=tau, w=np.array([3.0, 4.0]), P_c=1.0
    )
    np.testing.assert_array_equal(explicit.w, [3.0, 4.0])
    assert explicit.metadata["w_mode"] == "explicit_spatial"

    fallback = compute_agencity_field(u, t, A_ref=1.0, tau=tau, P_c=1.0)
    np.testing.assert_array_equal(fallback.w, tau)
    assert fallback.metadata["w_mode"] == "fallback_w_equals_tau"
    assert fallback.metadata["w_resolution"] == (
...
def test_constant_local_trajectory_is_exact_rest_without_affecting_neighbor():
    ...
    field = compute_agencity_field(u, t, A_ref=1.0, tau=4.0, w=3.0, P_c=2.0)
    np.testing.assert_array_equal(field.u_star[:, 0], 7.0)
    for name in REST_ZERO_FIELDS:
        np.testing.assert_array_equal(getattr(field, name)[:, 0], 0)
...
def test_generic_spatial_dimensions(spatial_shape):
    ...
    for index in np.ndindex(spatial_shape):
        u[(slice(None), *index)] = _signal(t, 0.03 * sum(index))
    ...
    assert field.beta.shape == field.b.shape == u.shape
    assert len(field.spatial_axes) == len(spatial_shape)


def test_time_axis_minus_one_is_permutation_equivalent():
    t = _time()
    first_input = np.empty((t.size, 2, 3), dtype=float)
    for index in np.ndindex((2, 3)):
        first_input[(slice(None), *index)] = _signal(t, 0.1 * sum(index))
    last_input = np.moveaxis(first_input, 0, -1)
    first = compute_agencity_field(
        first_input, t, A_ref=1.0, tau=4.0, w=3.0, P_c=2.0, time_axis=0
    )
    last = compute_agencity_field(
        last_input, t, A_ref=1.0, tau=4.0, w=3.0, P_c=2.0, time_axis=-1
    )
    for name in CANONICAL_FIELDS:
        np.testing.assert_allclose(
...
def test_default_spatial_axes_and_temporal_crm_metadata():
    t = _time()
    field = compute_agencity_field(
...
def test_local_translation_sign_and_power_invariances_are_inherited():
    ...
    scaled_power = compute_agencity_field(u, t, A_ref=1.0, tau=4.0, w=3.0, P_c=6.0)
